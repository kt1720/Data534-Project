...
# data preparing for each of the data frames `records2019` to `records 2022`
...
for(i in seq_along(r_list)){
  max_length <- max(max_length, length(r_list[[i]]))
}
max_length
column_names_table <- data.frame()
column_names_list <- list()
for(i in seq_along(r_list)){
  c <- c(colnames(r_list[[i]]), rep(NA, max_length-length(r_list[[i]])))
  column_names_list[[i]]<- c
}
column_names_table<- as.data.frame(column_names_list, col.names = c('2019', '2020', '2021', '2022', '2023'))
...
# for the reference period is not necessarily the start of the year, do not use the code below
...
# for annual_wage_flag_salaire_annuel==0, calculate yearly wages
df <- df_split%>%
  rowwise()%>%
...
# two economic regions called Northeast, one in ON, one in BC
# groupby(prov, er_code) necessary
...
#too many titles, outliers visualized infeasible 

# check duplicates and delete them
duplicated_rows <- function(df){
  duplicated_rows <- df%>%
  group_by(prov, er_code_code_re, reference_period, noc_title_eng, id)%>%
  mutate(duplicate_count=n())%>%
  filter(duplicate_count>1)
  return(duplicated_rows)
}
duplicates_check <- duplicated_rows(df)
# no duplicates

df_unique <- df%>%
  distinct(prov, er_code_code_re, reference_period, noc_title_eng, id, .keep_all = TRUE)
duplicates_check <- duplicated_rows(df_unique)
...
df_without_outliers <- df_unique%>%
  ...
  ungroup()
#127082-8573=118509 observations
# outliers removed

# example: trying to figure out plots here
df_2022_test <- df_unique%>%
...
average_median_bar_plot <- function(){
  ...
  ggplot(average_median)+
    aes(x=prov, y=median_wage_salaire_median)+
    geom_bar(stat = 'identity', fill ='skyblue')+
    labs(title = paste("average median salaries of", selected_noc_title, sep=' '),
         x="Province",
